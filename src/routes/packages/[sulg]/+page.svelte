<script lang="ts">
  ...
  const installCommand = $derived(`zig fetch --save git+${pkg.repositoryUrl}${pkg.version !== 'latest' ? '#' + pkg.version : ''}`);
...
<div class="container mx-auto">
    ...
    <div class="flex flex-wrap items-start justify-between gap-4">
      <div>
        <div class="flex flex-wrap items-center gap-3 mb-2">
          ...
          <span class="text-xs font-semibold px-2.5 py-1 rounded-full bg-yellow-100 text-yellow-700 border border-yellow-200">{pkg.version}</span>
          <span class="text-xs font-medium px-2.5 py-1 rounded-full bg-green-50 text-green-600 border border-green-200">{pkg.packageType}</span>
        </div>
        <p class="text-slate-500 text-lg max-w-2xl">{pkg.description}</p>
      </div>

      <div class="flex items-center gap-3 mt-1">
        <a href={pkg.repositoryUrl} target="_blank" rel="noopener" class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-slate-700 hover:border-yellow-400 hover:text-yellow-700 transition-colors">
  ...
  <div class="border-b border-gray-200 mb-8">
    <nav class="flex gap-0 -mb-px">
      ...
      >Versions {#if data.tags.length > 0}<span class="text-xs bg-gray-100 text-slate-500 px-2 py-0.5 rounded-full">{data.tags.length}</span>{/if}</button>
      <button
      ...
      >Dependencies {#if data.dependencies.length > 0}<span class="text-xs bg-gray-100 text-slate-500 px-2 py-0.5 rounded-full">{data.dependencies.length}</span>{/if}</button>
    </nav>
  </div>
  ...
  <div class="flex flex-col lg:flex-row gap-8">
    <div class="flex-1 min-w-0">
          ...
          {#if data.files.length > 0}
            <div class="border border-gray-100 rounded-xl overflow-hidden">
              {#each data.files as item (item.path)}
                <a href={item.htmlUrl} target="_blank" rel="noopener" class="flex items-center gap-3 px-4 py-2.5 hover:bg-gray-50 transition-colors border-t border-gray-100 first:border-t-0">
                  {#if item.type === 'dir'}
        ...
        <div class="bg-white border border-gray-200 rounded-2xl p-6 space-y-3">
          ...
          {#if data.tags.length > 0}
            {#each data.tags as tag, i (tag.name)}
              <div class="flex items-center justify-between py-3 px-4 rounded-xl hover:bg-gray-50 transition-colors {i === 0 ? 'bg-yellow-50/50 border border-yellow-100' : ''}">
                <div class="flex items-center gap-3">
                  <span class="font-mono font-semibold text-slate-900">{tag.name}</span>
                  {#if i === 0}
                    <span class="text-xs font-semibold px-2 py-0.5 rounded-full bg-yellow-100 text-yellow-700">latest</span>
                  {/if}
                </div>
                <span class="text-xs font-mono text-slate-400">{tag.sha.slice(0, 7)}</span>
              </div>
            {/each}
          {:else}
      ...
      {:else if activeTab === 'dependencies'}
        <div class="bg-white border border-gray-200 rounded-2xl p-6">
          <h2 class="text-lg font-bold text-slate-900 mb-4">Dependencies <span class="text-sm font-normal text-slate-400">({data.dependencies.length})</span></h2>
          {#if data.dependencies.length > 0}
            <div class="space-y-3">
              {#each data.dependencies as dep (dep.name)}
                <div class="flex items-center justify-between py-3 px-4 rounded-xl hover:bg-gray-50 border border-gray-100 transition-colors">
                  <div>
                  ...
                  {#if dep.hash}
                    <span class="text-xs font-mono text-slate-400 shrink-0 ml-4" title={dep.hash}>
                      {dep.hash.slice(0, 12)}...
                    </span>
                  {/if}
                </div>
    ...
    <aside class="w-full lg:w-80 shrink-0 space-y-5">
      <div class="bg-white border border-gray-200 rounded-2xl p-5">
        ...
        <div class="relative group">
          <div class="bg-slate-900 rounded-xl px-4 py-3 font-mono text-sm text-slate-100 overflow-x-auto">
            <span class="text-slate-500 select-none">$ </span>zig fetch --save <span class="text-yellow-300">git+{pkg.repositoryUrl}{pkg.version !== 'latest' ? '#' + pkg.version : ''}</span>
          </div>
          <button
            onclick={copyInstall}
      ...
      <div class="bg-white border border-gray-200 rounded-2xl p-5 space-y-4">
        ...
        <div class="space-y-2.5">
          <a href={pkg.repositoryUrl} target="_blank" rel="noopener" class="flex items-center gap-2.5 text-sm text-slate-600 hover:text-yellow-600 transition-colors">
          ...
          {#if pkg.homepage}
            <a href={pkg.homepage} target="_blank" rel="noopener" class="flex items-center gap-2.5 text-sm text-slate-600 hover:text-yellow-600 transition-colors">
              <svg class="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
              Homepage
            </a>
          {/if}
          <a href="{pkg.repositoryUrl}/issues" target="_blank" rel="noopener" class="flex items-center gap-2.5 text-sm text-slate-600 hover:text-yellow-600 transition-colors">
        ...
        <div class="space-y-3.5">
          <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-1.5">
              ...
              <span class="text-sm font-bold text-slate-900">{pkg.stars.toLocaleString()}</span>
            </div>
          </div>
          <div class="border-t border-gray-100 pt-3 flex items-center justify-between">
            <span class="text-sm text-slate-500">Forks</span>
            <span class="text-sm font-bold text-slate-900">{pkg.forks.toLocaleString()}</span>
          </div>
          <div class="border-t border-gray-100 pt-3 flex items-center justify-between">
            <span class="text-sm text-slate-500">Open Issues</span>
            <span class="text-sm font-bold text-slate-900">{pkg.openIssues.toLocaleString()}</span>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white border border-gray-200 rounded-2xl p-5">
        ...
        <div class="space-y-3.5">
          <div class="flex items-center justify-between">
            ...
            <span class="text-sm font-semibold text-slate-900">{pkg.version}</span>
          </div>
          <div class="border-t border-gray-100 pt-3 flex items-center justify-between">
            <span class="text-sm text-slate-500">License</span>
            <span class="text-sm font-semibold text-slate-900">{pkg.license || 'Unknown'}</span>
          </div>
          {#if data.zonInfo?.minimumZigVersion}
            <div class="border-t border-gray-100 pt-3 flex items-center justify-between">
              <span class="text-sm text-slate-500">Min Zig Version</span>
              <span class="text-sm font-semibold text-slate-900">{data.zonInfo.minimumZigVersion}</span>
            </div>
          {/if}
          <div class="border-t border-gray-100 pt-3 flex items-center justify-between">
            <span class="text-sm text-slate-500">Last Updated</span>
            <span class="text-sm font-semibold text-slate-900">{formatDate(pkg.pushedAt)}</span>
          </div>
          <div class="border-t border-gray-100 pt-3 flex items-center justify-between">
            <span class="text-sm text-slate-500">Created</span>
            <span class="text-sm font-semibold text-slate-900">{formatDate(pkg.createdAt)}</span>
          </div>
        </div>
      </div>

      <div class="bg-white border border-gray-200 rounded-2xl p-5">
        <h3 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">Owner</h3>
        <a href="https://github.com/{pkg.owner}" target="_blank" rel="noopener" class="flex items-center gap-3 group">
          {#if pkg.ownerAvatarUrl}
            <img src={pkg.ownerAvatarUrl} alt={pkg.owner} class="w-8 h-8 rounded-full" />
          {:else}
            <div class="w-8 h-8 rounded-full bg-linear-to-br from-yellow-400 to-orange-400 flex items-center justify-center text-white text-xs font-bold">{pkg.owner[0].toUpperCase()}</div>
          {/if}
          <span class="text-sm font-medium text-slate-700 group-hover:text-yellow-600 transition-colors">{pkg.owner}</span>
        </a>
      </div>
        ...
        <div class="bg-white border border-gray-200 rounded-2xl p-5">
          ...
          <div class="flex flex-wrap gap-2">
            {#each pkg.topics as topic (topic)}
              <a href="/packages?search={topic}" class="text-xs font-medium px-3 py-1.5 rounded-lg bg-gray-50 text-slate-600 border border-gray-100 hover:bg-yellow-50 hover:text-yellow-700 hover:border-yellow-200 transition-colors">{topic}</a>
            {/each}
          </div>
        </div>
      {/if}
