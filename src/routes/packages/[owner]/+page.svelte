...
<div class="text-slate-900 font-sans selection:bg-yellow-200 selection:text-black">
  <!-- Owner Header -->
  <div class="flex items-center gap-4 mb-8">
    {#if data.ownerAvatarUrl}
      <img
        ...
        alt={data.owner}
        class="w-16 h-16 rounded-2xl border border-gray-200"
      />
    {/if}
    <div>
      <h1 class="text-3xl font-extrabold text-slate-900">{data.owner}</h1>
      <p class="text-slate-500 mt-0.5">{data.totalCount} package{data.totalCount !== 1 ? 's' : ''}</p>
    </div>
  </div>
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
    {#each data.packages as pkg (pkg.fullName)}
      ...
      >
        ...
        <p class="text-sm text-slate-500 mb-6 grow leading-relaxed">{pkg.description}</p>
